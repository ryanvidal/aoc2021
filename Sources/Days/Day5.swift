...
public class Day5Solver: DailySolver {
    public typealias CalculationInput = [Instruction]
    ...
    public func PerformPart1Calculation(_ input: CalculationInput) -> Double? {
        ...
        for instruction in input {
            guard instruction.start.0 == instruction.end.0 || instruction.start.1 == instruction.end.1 else {
                continue
            }
            let xCoordinates = [instruction.start.0, instruction.end.0].sorted()
            let yCoordinates = [instruction.start.1, instruction.end.1].sorted()
            for i in xCoordinates[0]...xCoordinates[1] {
                for j in yCoordinates[0]...yCoordinates[1] {
                    if map[i] != nil {
                        if map[i]![j] != nil {
                            map[i]![j]! += 1
                        } else {
    ...
    public func PerformPart2Calculation(_ input: CalculationInput) -> Double? {
        ...
        for instruction in input {
            if instruction.start.0 == instruction.end.0 || instruction.start.1 == instruction.end.1 {
                let xCoordinates = [instruction.start.0, instruction.end.0].sorted()
                let yCoordinates = [instruction.start.1, instruction.end.1].sorted()
                for i in xCoordinates[0]...xCoordinates[1] {
                    for j in yCoordinates[0]...yCoordinates[1] {
                ...
                let xValues: [Int] = {
                    if instruction.start.0 > instruction.end.0 {
                        return Array(instruction.end.0...instruction.start.0).reversed()
                    } else {
                        return Array(instruction.start.0...instruction.end.0)
                    }
                }()
                let yValues: [Int] = {
                    if instruction.start.1 > instruction.end.1 {
                        return Array(instruction.end.1...instruction.start.1).reversed()
                    } else {
                        return Array(instruction.start.1...instruction.end.1)
                    }
                }()
                for (index, xValue) in xValues.enumerated() {
                    let yValue = yValues[index]
                    if map[xValue] != nil {
                        if map[xValue]![yValue] != nil {
                            map[xValue]![yValue]! += 1
                        } else {
                            map[xValue]![yValue] = 1
                        }
