...
public class Day8Solver: DailySolver {
    public typealias CalculationInput = [Day8Entry]

    public func ParseInput(_ input: String) -> CalculationInput {
        let inputEntryStrings = input.components(separatedBy: "\n")
...
public struct Day8Entry {
    let patterns: [Set<String.Element>]
    let display: [Set<String.Element>]

    init(_ input: String) {
        let inputComponents = input.components(separatedBy: " | ")
        self.patterns = inputComponents[0].components(separatedBy: " ").map { Set($0) }
        self.display = inputComponents[1].components(separatedBy: " ").map { Set($0) }
    }

    var numberOfEasyCharactersInDisplay: Int {
        return display.count { character in
    ...
    var decodedDisplayValue: Int {
        var p = [
            1: patterns.first { $0.count == 2 }!,
            4: patterns.first { $0.count == 4 }!,
            7: patterns.first { $0.count == 3 }!,
            8: patterns.first { $0.count == 7 }!,
        ]

        p[3] = patterns.first { pattern in pattern.count == 5 && p[7]!.isSubset(of: pattern) }!
        p[5] = patterns.first { pattern in pattern.count == 5 && p[4]!.filter { !p[1]!.contains($0) }.isSubset(of: pattern) }!
        p[2] = patterns.first { pattern in pattern.count == 5 && pattern != p[3] && pattern != p[5] }!
        p[9] = patterns.first { pattern in pattern.count == 6 && p[4]!.isSubset(of: pattern) }!
        p[0] = patterns.first { pattern in pattern.count == 6 && p[7]!.isSubset(of: pattern) && pattern != p[9] }!
        p[6] = patterns.first { pattern in pattern.count == 6 && pattern != p[9] && pattern != p[0] }!

        return display.reduce(0) { (acc, character) in 10 * acc + p.first { $0.value == character }!.key }
    }
}
